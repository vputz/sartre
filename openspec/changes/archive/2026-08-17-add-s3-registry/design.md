...
```
The SQL backend is bounded by transaction/row visibility (drop deletes the manifest row → a concurrent `set_pointer`'s manifest check sees it gone → `NotFound`; or the pointer insert is seen by drop → `Conflict`). A **transaction-less object store has no such serialization**, so this needs an explicit, verified protocol.

**Resolved mechanism (verified in `model/S3Drop.tla`): tombstone-before-delete + resolve-skips-tombstoned.** The candidate `dropping/<v>` marker was **rejected** — modelling exposed why it can't work: a pointer event is **append-only and immutable**, so once a promoter's event lands it cannot be retracted; no amount of marker-checking *prevents* the dangling event, because the promoter's marker-check and its event-append are two non-atomic S3 operations. So instead of preventing the dangling event, the closure makes it **harmless**: `drop_version` writes the `tombstones/<v>` marker **before** deleting the manifest, and `head`/`resolve`/`list_pointers` **skip a tombstoned target** — a pointer whose tail event references a reclaimed version reverts to its last valid version (or unset), never resolving to a missing manifest. The TLA model pins both halves as load-bearing (a config dropping either one produces a counterexample) and confirms the whole is non-vacuous (a promote can still succeed and GC can still reclaim). `set_pointer` additionally refuses to advance to a tombstoned version (matching the model's promoter precondition), reducing — though not needing to eliminate — dangling events.
...
- **Drop-guard mechanism** — RESOLVED to tombstone-before-delete + resolve-skips-tombstoned (see Decisions); the `dropping/<v>` marker was rejected because append-only events can't be retracted. Verified in `model/S3Drop.tla` (ok config passes non-vacuously; both `bad_noskip` and `bad_order` configs yield counterexamples).
- **Where `open_s3` lives** — RESOLVED: self-contained in the `s3-registry` capability, alongside (not merged into) `open_local`/`open_cloud`.
